...
function setupMegaMenu(){
 ...
 <div class="mega-col"><h4>ديكور</h4><a href="products.html?category=decor&sub=wall">ديكور منزل</a><a href="products.html?category=decor&sub=clock">ساعات حوائط</a></div>
...
function setupMobileMenu(){
 document.querySelectorAll('.header').forEach(header=>{
  ...
  if(!nav||!menu||nav.querySelector('.menu-toggle'))return;
  const toggle=document.createElement('button');
  toggle.className='menu-toggle';toggle.type='button';toggle.setAttribute('aria-label','فتح القائمة');
  toggle.innerHTML='<span></span><span></span><span></span>';
  nav.insertBefore(toggle,nav.firstChild);
  const panel=document.createElement('div');panel.className='mobile-menu';
  let html='';
  menu.childNodes.forEach(node=>{
   if(node.nodeType!==1)return;
   if(node.tagName==='A'){html+=`<a href="${node.getAttribute('href')}">${node.textContent.replace('▾','').trim()}</a>`}
   else if(node.classList.contains('has-mega')){const a=node.querySelector('a');if(a)html+=`<a href="${a.getAttribute('href')}">${a.textContent.replace('▾','').trim()}</a>`}
  });
  html+='<a class="wa-link" target="_blank" href="https://wa.me/201003956998">تواصل واتساب</a>';
  panel.innerHTML=html;
  header.appendChild(panel);
  toggle.addEventListener('click',()=>{toggle.classList.toggle('open');panel.classList.toggle('open')});
  panel.querySelectorAll('a').forEach(a=>a.addEventListener('click',()=>{toggle.classList.remove('open');panel.classList.remove('open')}));
 });
}
